...
[assignSensitivityLabel](https://learn.microsoft.com/en-us/graph/api/driveitem-assignsensitivitylabel) is a **protected API**, so access has to be [requested from Microsoft](https://learn.microsoft.com/en-us/graph/metered-api-list) on top of permissions, consent, and billing. Nothing in this utility can grant it. If writes are refused once everything below is complete, that approval is the likely reason rather than a mistake in the setup.
