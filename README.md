...
[assignSensitivityLabel](https://learn.microsoft.com/en-us/graph/api/driveitem-assignsensitivitylabel) is documented as a **protected API**, which Microsoft defines as needing validation beyond permissions and consent. For this API that validation is the Azure billing association described above. The [metered API list](https://learn.microsoft.com/en-us/graph/metered-api-list) notes that *some* protected APIs additionally require a submitted request, but it documents none for `assignSensitivityLabel`: that row links only to the billing setup, and the [setup procedure](https://learn.microsoft.com/en-us/graph/metered-api-setup) asks only for an application registration, an Azure subscription, and Contributor plus application-owner rights. The Teams APIs that did need a request stopped being metered on 25 August 2025.

So a write refused once permissions, consent, and billing are all in place usually has a cause documented elsewhere: a token issued before the billing association existed, which is why a fresh sign-in is needed after linking; a public-client connection; a tenant outside the global cloud; a locked or double-key-encrypted file; or an IRM-protected label that cannot be applied in app-only mode.
