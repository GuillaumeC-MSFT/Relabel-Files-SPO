...
2. **Grants administrator consent and assigns an owner**, from inside the utility. Registering creates the *application*; app-only sign-in additionally needs a **service principal** in the tenant, and Microsoft requires the Azure billing operator to own the application registration. During this workflow, the utility adds the signed-in Graph administrator as an application owner, reads the owners back to verify that assignment, and creates or reads the service principal before assigning each requested application permission. Use that same account for Azure CLI when it also has Contributor or Owner on the subscription; otherwise add the intended billing operator as another application owner. Permissions and ownership already present are treated as success, so this step is safe to repeat. If you decline, or it fails, the utility describes both portal steps instead. It deliberately does **not** link to the `/adminconsent` endpoint, which requires a registered `redirect_uri` that a certificate-only application does not have and would land on an error page.
...
- Every operation performed through the **Microsoft.Graph** or **Az PowerShell** modules runs in a **separate PowerShell process**. This is required because `PnP.PowerShell`, `Az.Resources`, and `Microsoft.Graph.Authentication` can load incompatible versions of the same .NET assemblies, and .NET cannot replace an assembly after it has been loaded. Keeping Az and Microsoft.Graph modules out of the process that hosts PnP removes that class of failure. The Azure CLI route is already a separate executable, so the main process invokes it directly; every relevant command receives `--subscription` explicitly and does not change an existing CLI default subscription. The isolated Graph and Az workers reuse their own saved sign-ins, so isolation does not add prompts.
- Registering an application announces the sign-in it is about to open and honors `-DeviceLogin`, so device-code authentication can be chosen when browser windows are inconvenient.
